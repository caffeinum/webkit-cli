...
# Popup variant: SIGNIN_URL=http://127.0.0.1:8765/signin-popup GOOGLE_BUTTON='text=Sign in' rehearse-browser-use.sh …
set -eu
cd "$(dirname "$0")"
profile=$1
...
chmod +x "$wrapper"
WEBKIT_CLI=$wrapper SIGNIN_URL=${SIGNIN_URL:-http://127.0.0.1:8765/signin} GOOGLE_BUTTON=${GOOGLE_BUTTON:-'text=Continue with IdP'} SIGNED_OUT_RE='/signin' \
...
API_KEYS_URL=http://127.0.0.1:8765/dashboard CREATE_BUTTONS='#create' TOKEN_RE='sk-qa-[0-9a-f]{24}' \
