...
- `--human-timeout <sec>` (default 600) is how long an escalation waits for the human. When it runs out → exit 3 with stderr saying no one finished the step. In every case (plain `wait --until-hidden` too) the window is **hidden again** when that happens, so nobody walks back to a stale window; the tab stays alive. `--timeout` doesn't include time spent waiting on the human.
...
| E8 | **queue + idle** | (pm decision 2026-09-23) `tabs`, `hide`, `close` and `stop` are control commands: they **skip the queue** and work while an `--escalate`/`wait --until-hidden` is blocking. `close`/`stop` on the waited-on tab make that wait exit 1 ("closed before Done"). Error messages only suggest ways that actually work. The idle timer never fires while any tab is shown. |
