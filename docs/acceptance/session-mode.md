...
| A5 | **idle expiry mid-flow** | start session with `--idle 5` → `open` → sleep 10 → `text <tab>` | the daemon is gone (no process, no socket). The error names the missing tab and says sessions expire when idle — exit 1, not a hang. The next `open` starts a fresh session. The idle shutdown starts (the `exiting: idle` log line) between `idle` and `idle + min(idle, 30s)` after the last request returns (with `--idle 5`: 5–10s; with the default 900: 900–930s). The process exits and the socket is gone within 2s after that log line. A command running longer than idle (`wait --timeout 20` with `--idle 5`) is **not** killed mid-command. |
| A6 | **two clients, one daemon (requests run one at a time)** | two shells at once: shell 1 `wait $t --until-selector '#late' --timeout 30` against `/late?s=10`, shell 2 `open`, `text`, `tabs` on the same profile. Then shell 1 `wait … --timeout 30` against `/late?s=60` and shell 2 `text $t --timeout 5` | exactly one daemon pid for the profile (`pgrep`). Requests run in arrival order: shell 2 finishes after shell 1's wait (~10s) and both results are correct, with no spurious timeout. When a command waits longer in the queue than its own `--timeout` allows → exit 3, and stderr says it was queued behind another request (not a generic socket error). The idle timer never fires while requests are queued. 5 parallel `open`s → 5 distinct, non-sequential tab ids, one daemon. Two parallel first commands (cold start race) → still one daemon. |
...
| A13 | **stop / close / tabs** | `close $t` → `tabs`; `stop` → `pgrep`; `stop` again | tab gone; daemon gone and cookies saved; a second `stop` exits 0 or clearly says there's no session (no crash). Bad tab id `x9` → exit 2. |
...
**parity with bench:** the script reaches the same end state as Search's bench on this flow (signed in → dashboard → key created) with no extra manual steps. A step bench handles that the script can't (account chooser, consent, popup variant, slow hydration) is a fail. Dev lists in the script header which bench behaviours were ported.
